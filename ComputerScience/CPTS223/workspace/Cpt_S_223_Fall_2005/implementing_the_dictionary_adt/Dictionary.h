...
cPosition<Key, Element> Dictionary<Key,Element>::find(const Key k)
{
   ...
   {
      if (mDictionary[i].key() == k)
      {
         temp.mPosition = &(this->mDictionary[i]);
         return temp;
      }
   }   
   return temp;
...
{
   PositionIterator temp;
   for (unsigned int i = 0; i < mDictionary.size(); i++)
   {
      if (mDictionary[i].mKey == k)
      {
         temp.mPositionIterator.push_back(&(mDictionary[i]));
      }
   }
   return temp;
}
...
void Dictionary<Key,Element>::removeElement(const Key k)
{
   ...
   for (unsigned int i = 0; i < mDictionary.size(); i++)
   {
      if (mDictionary[i].mKey != k && is_erased == false)
      {
         temp.push_back(mDictionary[i]);
      }
   }
//   if (is_erased == false)
//   {
//      throw (NonexistentElementException("Not there cant remove") );
//   }
   this->mDictionary = temp;
}
...
{
   ElementIterator temp;
   for (unsigned int i = 0; i < mDictionary.size(); i++)
   {
      (temp.mElementIterator).push_back(mDictionary[i].element());
   }
   return temp;
}
...
cKeyIterator<Key, Element> Dictionary<Key,Element>::keys()
{
   ...
   {
      k = mDictionary[i].key();
      (temp.mKeyIterator).push_back(mDictionary[i].key());
   }
   return temp;
}
...
{
   KeyIterator a = this->keys();
   ElementIterator b = this->elements();
