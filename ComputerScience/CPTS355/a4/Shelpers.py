...
# global ints for static use passing between fucntions
counter = 0
index = 0
staticLink = 0
last = 0
...
#		it will also modify the stack by pushing the susdeclarationpended value onto the stack 
#9/24/2007
...
def priorDef(name,dictionaryStack):
	...
	index = index - 1
	help = 1
	#print "NAME DICTSTACK INDEX", name, dictionaryStack, index
	while (index > -1):
		#print dictionaryStack[index], index, name
		if dictionaryStack[index][1].has_key(name):
			#print "returning true"
			return True
		else:
			index = dictionaryStack[index-1][0]			
		if index == 0 and help == 0:
			return False 
		elif index == 0:
			help = 0
...
def defineName(name,dictionaryStack):
	global index
	return dictionaryStack[index][1][name]
